...
export default function RepositorySearchPage() {
  ...
  const searchParams = submittedQuery
    ? { repositoryId: repoId, q: submittedQuery }
    : null;
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title={`Search — ${repo?.name ?? "Repository"}`}
      ...
      {!submittedQuery && (
        <Card className="flex flex-col items-center gap-3 px-6 py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground">
            Enter a query above to search this repository&rsquo;s codebase.
          </p>
        </Card>
      )}
    </div>
